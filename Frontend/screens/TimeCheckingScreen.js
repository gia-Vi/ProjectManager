...
const fixedLocation = { latitude: 37.4220936, longitude: -122.083922 };
...
const TimeTrackingScreen = () => {
    ...
    const handleCheckIn = async () => {
      if (location) {
        if (location.coords.latitude === fixedLocation.latitude && location.coords.longitude === fixedLocation.longitude) {
          ...
          Alert.alert('Error', 'You are not at the your company.');
        }
      } else {
  ...
  // };
   ...
   // Kiểm tra vị trí và hiển thị nút cho phép thực hiện hành động
  const handleCheckLocation = () => {
    if (location) {
      if (location.coords.latitude === fixedLocation.latitude && location.coords.longitude === fixedLocation.longitude) {
        Alert.alert('Success', 'You are at the fixed location.');
      } else {
        Alert.alert('Error', 'You are not at the fixed location.');
      }
    } else {
      Alert.alert('Error', 'Cannot determine your current location.');
    }
